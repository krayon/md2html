...
#/**********************************************************************
#    md2html
#    Copyright (C)2019-2022 Krayon (Todd Harbour)
#
...
# Markdown 2 HTML - Default Configuration
# =======================================
...
# TEMPLATE
#   Template file used for generation. If unset, the following will be used if
#   they exist, in the following order:
#     * ${HOME}/.config/md2html/TEMPLATE.html
#     * ${HOME}/.config/md2html.TEMPLATE.html
#     * ${HOME}/.md2html.TEMPLATE.html
#     * /etc/md2html.TEMPLATE.html
#   If set to "", no template will be used
#
#   The template can contain various "special" strings that will be replaced at
#   generation time:
#     * %%%%%BODY%%%%%    - The body that gets generated
#     * %%%%%TITLE%%%%%   - The title of the page (derived from first top level
#                           heading)
#     * %%%%%WARNING%%%%% - A warning saying the file was generated by this
#                           application and that it should not be manually
#                           modified.
#TEMPLATE="${HOME}/.config/md2html/TEMPLATE.html"
#TEMPLATE="${HOME}/.config/md2html.TEMPLATE.html"
#TEMPLATE="${HOME}/.md2html.TEMPLATE.html"
#TEMPLATE="/etc/md2html.TEMPLATE.html"

# FILES
#   An array of files to process, in the format:
#       FILES=(
#           '/myfiles/input1.md'
#           '/myfiles/input2.md'
#       )
#   If set, these files will be processed. Can also be provided on the command
#   line.
FILES=()
...
APP_VER="0.01"
APP_COPY="(C)2019-2022 Krayon (Todd Harbour)"
#TODO:
#APP_URL="https://github.com/krayon/qdnxtools/md2html.bash"
...
#   $4 =  (i) is this a required package? (1 = yes, 0 = no)
...
# consts
warning="WARNING: THIS FILE IS GENERATED BY ${_binname} - CHANGES MAY BE LOST"
...
PATH_MARKDOWN_PY="$(check_for_cmd "markdown_py" "${PATH_MARKDOWN_PY}" 1 1 "python-markdown")" || exit $?
...
template=""
! declare -p "TEMPLATE" &>/dev/null && {
    [   -z "${template}" ] && template="${HOME}/.config/md2html/TEMPLATE.html"
    [ ! -r "${template}" ] && template=""

    [   -z "${template}" ] && template="${HOME}/.config/md2html.TEMPLATE.html"
    [ ! -r "${template}" ] && template=""

    [   -z "${template}" ] && template="${HOME}/.md2html.TEMPLATE.html"
    [ ! -r "${template}" ] && template=""

    [   -z "${template}" ] && template="/etc/md2html.TEMPLATE.html"
    [ ! -r "${template}" ] && template=""
...
process_file() {
    ...
    [ ! -z "${template}" ] && [ -r "${template}" ] && {
        sed '/^/,/^%%%%%BODY%%%%%/{/^%%%%%BODY%%%%%/,$d}' <"${template}" >"${tmpdir}/BEFORE"
        sed '1,/^%%%%%BODY%%%%%/d'                        <"${template}" >"${tmpdir}/AFTER"
        sed -i 's/%%%%%TITLE%%%%%/'"${title}"'/g' "${tmpdir}/BEFORE"
        sed -i 's/%%%%%TITLE%%%%%/'"${title}"'/g' "${tmpdir}/AFTER"
        sed -i 's/%%%%%WARNING%%%%%/'"${warning}"'/g' "${tmpdir}/BEFORE"
        sed -i 's/%%%%%WARNING%%%%%/'"${warning}"'/g' "${tmpdir}/AFTER"
    }

    >"${outfile}"

    [ -r "${tmpdir}/BEFORE" ] && cat "${tmpdir}/BEFORE" >>"${outfile}"
    ...
    [ -r "${tmpdir}/AFTER"  ] && cat "${tmpdir}/AFTER"  >>"${outfile}"

    [ -r "${tmpdir}/BEFORE" ] && rm "${tmpdir}/BEFORE"
    [ -r "${tmpdir}/AFTER"  ] && rm "${tmpdir}/AFTER"
}
